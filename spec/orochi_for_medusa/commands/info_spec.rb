require 'spec_helper'
require 'orochi_for_medusa/commands/info'
include MedusaRestClient

module OrochiForMedusa::Commands
	describe Info do
		let(:cui) { Info.new(args, :stdin => stdin, :stdout => stdout, :stderr => stderr, :program_name => 'orochi-info') }
		let(:args){ [] }
		...
		describe "execute" do		
			...
			describe "without id" do
				...
				it {
					...
					expect(cui).to receive(:w3m).with(id)
					subject
				}
			end
