require 'spec_helper'
require 'orochi_for_medusa/commands/label'
include MedusaRestClient

module OrochiForMedusa::Commands
  describe Label do
    let(:cui) { Label.new(args, :stdin => stdin, :stdout => stdout, :stderr => stderr, :program_name => 'orochi-label') }
    let(:args){ [] }
    ...
    describe "execute" do   
      ...
      before do
        cui.parse_options
        allow(cui).to receive(:find_and_print)
      end
      ...
      describe "without id" do
        ...
        it {
          ...
          expect(cui).to receive(:find_and_print).with(id)
          subject
        }
      end
